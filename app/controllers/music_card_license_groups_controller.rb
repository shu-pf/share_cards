class MusicCardLicenseGroupsController < ApplicationController
  def new
  ...
  def create
    ...
    @license_group[:music_card_id] = card.id
    @license_group.save!
    ...
    redirect_to music_card_music_card_license_group_download_url(card, @license_group), notice: "カードが作成されました"
  end
  ...
  def download
    @card = MusicCard.find(params[:music_card_id])
